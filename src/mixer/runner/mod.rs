...
use super::{Channel, MixerSettings, MixerTask};
...
impl Runner {
    pub(super) fn new(settings: Arc<RwLock<MixerSettings>>) -> mpsc::Sender<MixerTask> {
        ...
        std::thread::spawn(move || {
            let period =
                Duration::from_millis(settings_thread.read().unwrap().buffer_refill_interval_ms);
            loop {
                std::thread::sleep(period);
                if tx_task_thread.send(MixerTask::RefillBuffer).is_err() {
                    break;
                }
            }
        });
        ...
        while let Ok(task) = rx_task.recv() {
            match task {
                MixerTask::Initialize => {
                    ...
                    runner.take();
                    runner = Some(Runner {
                        output: output::Output::new(settings.read().unwrap().buffer_size_ms)
                            .unwrap(),
                ...
                MixerTask::ChannelLoad {
                    ...
                    if let Some(runner) = runner.as_mut() {
                        if let Some(old) = runner
                            .channels
                            .insert(channel, Channel::new(decoder, resampler, time_stretcher))
                        {
                            // TODO: Drop stream
                        }
                    }
